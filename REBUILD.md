...
[WSL: server.py :8765]         ← Python標準ライブラリのみのHTTPサーバー
   ...
   ├─ POST /api/stop          → 実行中断
   ├─ GET  /api/sessions      → 保存済み会話の一覧 (履歴サイドバー用)
   ├─ GET  /api/session?sid=  → 1会話の全メッセージ (再開用)
   └─ POST /api/session/delete → 会話の削除
...
**ツール6種**: `run_command`（bash実行, cwd=作業フォルダ, 180秒タイムアウト）/
`read_file` / `write_file` / `list_dir`（ファイル系は作業フォルダ外へのアクセスを拒否）/
`web_search`（DuckDuckGo HTML版のスクレイピング、APIキー不要・無料）/
`fetch_url`（ページ取得→HTMLタグ除去テキスト、10KB上限）
...
2. **`OLLAMA_HOST=0.0.0.0` は自動化ポリシーで拒否された** — 「LAN全体にOllamaを公開する」
   ...
   New-NetFirewallRule -DisplayName "Ollama (WSL only)" -Direction Inbound -Protocol TCP `
     -LocalPort 11434 -Action Allow -InterfaceAlias "vEthernet (WSL)"
   ```

   IPアドレスではなく `InterfaceAlias` でスコープすることで、`vEthernet (WSL)` の
   IPが再起動で変わっても（WSL2 NATのIPは固定されない）ルールを再登録せずに済む。
   ファイアウォールルール追加はこの実施では管理者権限が無く自動実行できなかったため、
   コマンドをユーザーに提示して手動実行してもらった。
...
- `C:\Users\<user>\Desktop\LocalCoder.bat`: `powershell -NoProfile -File "<repoのUNCパス>\LocalCoder.ps1"`
  を呼ぶだけの薄いラッパー（`-ExecutionPolicy Bypass` は不要。`Get-ExecutionPolicy -List` で
  `LocalMachine: RemoteSigned` を確認済みなら、自作の未署名スクリプトでも素通りする）
