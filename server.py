...
UPDATE_SUMMARIZE_PROMPT = """以下は「これまでの会話の要約」と「その後に新しく起きたことの要約」である。
...
- 「新しい要約」で判明した内容(変更ファイル・技術的事実・完了/未完了)を追加すること
...
上記2つを統合し、更新後の要約を日本語で出力せよ。出力は要約本文のみ。"""


def merge_summaries(prev: str, new_part: str, model: str) -> str:
    """既存の要約(prev)と新規分の要約(new_part)を統合する。
    ...
    merged = ollama_ask(model, UPDATE_SUMMARIZE_PROMPT.format(
        prev=prev, log=new_part)).strip()
    return merged or prev
...
def compact_history(messages: list, model: str, sse) -> list:
    ...
    第2段階: 直近KEEP_RECENT_MSGS件を残して古い部分をLLMで要約し1メッセージに置換。
             ...
             再要約せず「これまでの要約」として引き継ぎ、新規分だけを要約してマージ
             する(世代劣化の防止)。変更ファイル一覧はLLMを介さず機械抽出して
             常にマーカーに引き継ぐ。
    ...
    try:
        if new_raw:
            new_part = summarize_old(new_raw, model)
            summary_text = merge_summaries(prev_summary, new_part, model) if prev_summary else new_part
        else:
