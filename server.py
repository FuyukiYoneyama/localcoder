...
def derive_title(messages: list) -> str:
    ...
    for m in messages:
        if m.get("role") == "user" and m.get("content"):
            c = m["content"]
            summary, _files, _pinned = _parse_marker(c)
...
def _parse_marker(content: str) -> tuple[str | None, list[str], list[str]]:
    ...
    マーカーでなければ (None, [], []) を返す。ファイル一覧・固定指示は要約本文とは
    別ブロックに保存されているため、LLMによる再要約でパラフレーズされず正確な値の
    まま読み戻せる。
    """
    ...
    return summary_part.strip(), files, pinned
...
def build_marker(summary_text: str, files: list[str], pinned: list[str] | None = None,
    ...
    ファイル一覧・固定指示はLLM要約の本文に混ぜず別ブロックとして常に付記する。
    機械的に検出した事実/原文なので、要約の質に関わらず正確な値のまま次回以降も
    引き継がれる。固定指示ブロックはファイル一覧ブロックより後方に置く(要約本文の
    直後にあるファイル一覧側のパース(partition)を崩さないため)。
    """
    if failed:
...
def compact_history(messages: list, model: str, sse, force: bool = False) -> list:
    ...
    prev_summary, prev_files, prev_pinned = _parse_marker(old[0].get("content", ""))
    ...
    try:
        ...
        marker = build_marker(summary_text, all_files, all_pinned)
    except Exception as e:
        # 新規分の要約に失敗しても、既存の要約(あれば)・ファイル一覧・固定指示は
        # 機械的に保持する。全て消すより情報を残す方が安全。
        note = f"(直近の新規会話部分の要約に失敗したため未反映: {type(e).__name__})"
        summary_text = f"{prev_summary}\n{note}" if prev_summary else note
        marker = build_marker(summary_text, all_files, all_pinned, failed=not prev_summary)
