...
ローカルLLM (Windows側 Ollama) 経由で動く、GUIコーディングエージェント。
codex / claude code のようにファイル読み書き・コマンド実行を全自動で行う。
LocalCoder自身は外部のLLM APIを直接呼び出さない——常にローカルのOllama
(`http://localhost:11434`)としか通信しない。ただし実際にどのモデルが応答する
かはOllama自体の挙動による: モデル一覧に`:cloud`サフィックス付きのモデルが
表示される場合、それを選ぶとOllamaがそのモデルの実処理を外部のクラウドへ
プロキシする(Ollamaのクラウドモデル機能)。完全ローカルで完結させたいなら、
`:cloud`が付かないモデルを選ぶこと。Python側は標準ライブラリのみで動作する。
web_search / fetch_url を使うと検索語やアクセス先URLはインターネットへ送信される。
...
`history/<sid>.json`は文脈予算を守るため継続的に圧縮される。圧縮で要約に
置き換えられた生ログは、コンテキストとは別に`history/raw/<sid>.jsonl`へ
1メッセージ1行で追記され続けている(`archive_raw_messages`、gitignore済み)。
セッションを検討するときは、まずこれと現在の`history/<sid>.json`を連結して
完全な非圧縮ログを組み立てること:

```bash
python3 tools/reconstruct_raw.py <sid> -o /tmp/<sid>_full.json
```

停滞・早すぎる介入等が疑われるセッションがあれば、その完全ログを
`tools/replay_review.py`にかけて、現在のコードによる方針再評価の発火タイミングを
機械的に(Ollama不要で)確認できる。

```bash
python3 tools/replay_review.py /tmp/<sid>_full.json
```

(圧縮が一度も起きていないセッションは`history/raw/`にファイルが無く、
`history/<sid>.json`自体が既に完全ログなので直接かけてよい。)
