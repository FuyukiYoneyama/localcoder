...
1ファイル（2600行超）に実装している。このドキュメントはソースを引用しながら、
起動時の処理順に沿って動作を説明する。
...
- `OLLAMA` / `PORT`（`server.py:34-35`）は環境変数で上書き可能。デフォルトは
  どちらもlocalhost想定。WSLからWindows側Ollamaへ別経路で繋ぐ場合は
  `LOCALCODER_OLLAMA=http://<IP>:11434` を指定（実例は `REBUILD.md` 「8. 実施例ログ」）。
- `SERVER_VERSION`（`server.py:39-60`）: `_detect_version()`が起動中のgitコミット
...
（実装は`server.py:2140-2183`付近、`_host_ok`/`_token_ok`/`_post_ok`の3メソッド）。
...
```
（`server.py:2249-2262`、`do_GET`全体は2184-2264）
...
if self.path == "/api/transaction/reapply":
    ...
    return
```
（`server.py:2285-2290`、`do_POST`全体は2267-2294）
...
`/api/transaction/rollback`・`/reapply`は`_handle_txn_action`（`server.py:2296-2314`）
...
**セルフチェック**（`run_self_check`, `server.py:375-434`）は起動時に1回だけ実行され、
...
**診断バンドル**（`build_diagnostic_bundle`, `server.py:437-502`）はバージョン・
...
入れる、`server.py:462`）。ユーザーが「🩺 診断」ボタンでダウンロードし、バグ報告に
添付する用途。
...
def handle_chat(self):
    ...
    txn = Transaction(wsr)
```
（`server.py:2316-2345`付近。`sys_prompt`はMCP接続時に追加ツールの説明を付記する
——`server.py:2350-2364`付近）
...
`save_session`（`server.py:547-572`）はタイトルを初回保存時に一度だけ確定し
（圧縮で先頭メッセージが要約マーカーに置き換わってもタイトルが変わらないように、
`server.py:564-568`）、`schema_version`（現在2）を付与する。
...
```
（`ToolContext`: `server.py:1109-1134`、`ToolProvider`: 1137-1146、
...
`BuiltinToolProvider`（`server.py:1149-1311`）が組み込み12ツールを振り分ける。
...
```
（`server.py:1188-1194`。`find_previous_read`: 10-3節）
...
`read_file`は`.pdf`拡張子を自動判別し`pdf_to_text()`（`server.py:505-524`）が
`pdftotext -layout`でテキスト抽出する。スキャン画像PDF等でテキストが取れない場合は
その旨を返す。`view_image`（`server.py:1288-1303`）は`model_capabilities()`
（`server.py:356-369`、`/api/show`の`capabilities`をモデル名でキャッシュ）で
...
def track_tool_repeat(name, args, result, last_failed_sig, repeat_count):
    ...
    打ち切る。"""
```
（`server.py:1091-1106`。`handle_chat`側の使用箇所でループを`break`する）
...
`Transaction`クラス（`server.py:661-828`）が1回の`/api/chat`リクエストを1つの
...
**原子的書き込み**（`atomic_write`/`atomic_write_bytes`, `server.py:626-647`）:
一時ファイル（`.{name}.localcoder-tmp`）へ書いてから`os.replace`で置き換える。
プロセス停止や書き込みエラーで対象ファイルが中途半端な内容になるのを防ぐ。

**台帳の保護**（`in_ledger_area`, `server.py:650-658`）: `.localcoder/`自体への
...
専用ツール4種が生の`rm`/`mv`より優先される（システムプロンプト`server.py:187`）。
...
```
（`server.py:737-763`、抜粋）

`move_file`/`copy_file`は移動元・移動先・上書きされた既存内容を記録する
（`record_move`/`record_copy`, `server.py:765-798`）。新規親ディレクトリの検出は
...
```
（`server.py:139-155`。git push / curl・wgetの送信系 / scp・sftp / rsync・sshの
...
```
（`server.py:854-931`、要旨）

`reapply_transaction`（`server.py:934-976`）はロールバック済みトランザクションの
...
が`TXN_ID_RE`でIDの形式も検証、`server.py:830-833`）。
...
```
（`server.py:2041-2124`付近）

処理は多段階:

1. **重複除去**（`dedupe_tool_results`, `server.py:1606-1629`）— 同一内容のツール
   結果は最新の1件だけ残し、古い方は短い参照文に置換する（LLM不使用）。弱いモデルが
   同じファイルを何度も読み直すと、履歴の半分が同一内容の重複だったセッションが
   実際にあった。
2. **古いツール結果の切り詰め**（`trim_old_tool_results`, `server.py:1630-1642`）—
   直近`KEEP_RECENT_TOOLS`(4)件を除くツール結果を500文字に切り詰める。
3. **要約**（`summarize_old`, `server.py:1767-1787`）— 直近`KEEP_RECENT_MSGS`(6)件を
...
- **変更ファイル一覧**（`extract_changed_files`, `server.py:1817-1843`）—
...
```
（`server.py:1971-2040`が`build_work_state`本体、呼び出し箇所は5-1節）

`_iter_tool_calls_with_results`（`server.py:1788-1816`）が`assistant`の
...
`server.py:1484-1510`）。`main()`（12節）が起動時に`TOOL_PROVIDERS`へ追加し、
...
（`server.py:2350-2364`付近。ローカルの弱いモデルは`tools`スキーマだけだと
追加ツールを使い忘れることがあるため）。
...
`run_command`（`server.py:1012-1043`）は`Popen(start_new_session=True)`で
...
`save_full_tool_output()`（`server.py:993-1009`）で完全な内容を
`history/tool_output/<sid>/<call_id>.txt`へ診断用に保存してから、モデルへは
先頭6KB＋末尾6KBに切り詰める。

`resolve_path`（`server.py:590-596`）はファイル操作ツール共通のサンドボックス。
`under_allowed`/`list_subdirs`（`server.py:599-622`）はワークスペース選択・
...
`web_search`（`server.py:299-317`）はDuckDuckGoのHTML版をUser-Agent偽装付きで
スクレイピングする軽量実装。`fetch_url_text`（`server.py:341-350`）は自前の
...
- [METACOGNITIVE_REPLANNING.md](METACOGNITIVE_REPLANNING.md) — 停滞・目的逸脱を
  検知して作業方針を自動再構築するメタ認知・再計画パス設計（未実装、計画のみ）
...
- [CHANGELOG.md](CHANGELOG.md) — 変更履歴。各エントリがコミットハッシュ付きで
  検証結果まで記録されている
- [tests/](tests/) — stdlib `unittest`のみの回帰テスト一式（195件超）。
...
- [index.html](index.html) — 本ドキュメントで扱ったSSEイベント（`think`/`token`/
  ...
  `history`/`all_done`/`error`）を受け取り、`window.LC_TOKEN`を全POST＋履歴系GETに
